...
# --- File Name: perceptual_path_length_v2.py
# --- Creation Date: 17-11-2020
# --- Last Modified: Tue 17 Nov 2020 16:25:35 AEDT
# --- Author: Xinqi Zhu
# .<.<.<.<.<.<.<.<.<.<.<.<.<.<.<.<
"""
Docstring
...
class PPLV2(metric_base.MetricBase):
    ...
    def _evaluate(self, Gs, num_gpus, **kwargs):
        ...
        for gpu_idx in range(num_gpus):
            with tf.device('/gpu:%d' % gpu_idx):
                ...
                with tf.control_dependencies([var.initializer for var in noise_vars]):  # use same noise inputs for the entire minibatch
                    try:  # StyleGAN
                        images = get_return_v(Gs_clone.components.synthesis.get_output_for(dlat_e01, is_validation=True,
                                                                              randomize_noise=False), 1)
                    except AttributeError:  # ProGAN
                        images = get_return_v(Gs_clone.get_output_for(dlat_e01, None, is_validation=True, randomize_noise=False), 1)
